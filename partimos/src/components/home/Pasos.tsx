    ...
    <Section id="como" stop>
      ...
      <ol className="mt-9 grid gap-3.5 min-[760px]:grid-cols-3 min-[760px]:gap-4.5">
        {PASOS.map((paso, index) => (
          <li
            key={paso.title}
            className="relative overflow-hidden bg-plate-50 p-6"
          >
            <span
              aria-hidden
              className="absolute top-3 right-4.5 text-[52px] leading-none font-extrabold tracking-[-0.05em] text-transparent [-webkit-text-stroke:2px_var(--color-ink-200)]"
            >
              {index + 1}
            </span>
            <span className="mb-4 flex size-11.5 items-center justify-center bg-white text-plate-900 shadow-[0_3px_10px_rgb(14_42_53/0.07)]">
