...
export function AccountSpace() {
  ...
  if (!session) {
    return (
      <Container className="pt-10">
        <div className="mx-auto max-w-[520px] border border-plate-200 bg-white p-7">
          ...
          <ul className="mb-6 grid gap-2.5">
            {TABS.map((t) => (
              <li
                key={t.id}
                className="flex items-center gap-3 bg-plate-50 px-4 py-3 text-[14.5px] font-medium"
              >
                <Icon
                  name={t.icon}
                  className="size-5 shrink-0 text-plate-600"
                />
                {t.label}
              </li>
            ))}
          ...
          <AuthDialog
            trigger={
              <button className="w-full bg-plate-900 px-7 py-4 text-[17px] font-bold text-white transition-colors hover:bg-plate-800">
                Entrar
              </button>
            }
          />
          <p className="mt-4 text-center text-[13px] text-plate-600">
  ...
  return (
    <Container className="pt-6">
      <div className="mb-5 flex flex-wrap items-center gap-4 border border-plate-200 bg-white p-5">
        <span
          aria-hidden
          className="bg-plate-800 flex size-14 shrink-0 items-center justify-center rounded-full text-[21px] font-bold text-white"
        >
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="cote text-[13.5px] text-plate-600">{session.contact}</p>
        </div>
        <button
          onClick={signOut}
          className="border-[1.5px] border-plate-200 px-4 py-2.5 text-[14px] font-semibold transition-colors hover:border-ochre-500 hover:text-ochre-600"
        >
          Salir
        </button>
      </div>
      ...
      >
        {TABS.map((t) => (
          <button
            ...
            id={`tab-${t.id}`}
            onClick={() => setTab(t.id)}
            className={`flex shrink-0 items-center gap-2 rounded-full border-[1.5px] px-4 py-2.5 text-[14px] font-semibold transition-colors ${tab === t.id ? "border-plate-900 bg-plate-900 text-white" : "border-plate-200 bg-white text-plate-600 hover:border-ochre-500"}`}
          >
      ...
      <div
        ...
        className="border border-plate-200 bg-white p-5 sm:p-6"
      >
        {tab === "viajes" && (
          <Empty
            icon="route"
        ...
        {tab === "perfil" && (
          <>
            <h2 className="mb-4 text-[19px] font-bold">Mi perfil</h2>
        ...
        {tab === "verificacion" && (
          <>
            <h2 className="mb-4 text-[19px] font-bold">Verificación</h2>
            ...
            {!session.isVerified && (
              <p className="mt-4 bg-plate-50 px-4 py-3 text-[13.5px] leading-relaxed text-plate-600">
      ...
      {isDemo && (
        <p className="mt-5 text-center text-[12.5px] text-plate-600">
...
function Empty({
  ...
  return (
    <div className="py-4 text-center">
      <span className="mx-auto mb-4 flex size-14 items-center justify-center rounded-full bg-plate-50 text-plate-600">
        <Icon name={icon} className="size-6" />
      </span>
      <h2 className="mb-2 text-[19px] font-bold">{title}</h2>
      <p className="mx-auto mb-5 max-w-[46ch] text-[14.5px] leading-relaxed text-plate-600">
        {body}
      </p>
...
function Field({
  ...
  return (
    <div className="flex items-baseline justify-between gap-4 border-b border-plate-200 pb-2.5 last:border-0">
      <dt className="text-[13.5px] text-plate-600">{label}</dt>
      <dd className="text-right">
        <span className="block text-[15px] font-semibold">{value}</span>
        {note && (
          <span className="block text-[12px] text-plate-600">{note}</span>
        )}
      </dd>
    </div>
  );
...
function Status({
  ...
  return (
    <li className="flex items-start gap-3 border border-plate-200 px-4 py-3">
      <span
        aria-hidden
        className={`mt-0.5 flex size-6 shrink-0 items-center justify-center rounded-full ${done ? "bg-plate-900 text-white" : "border-2 border-plate-200"}`}
      >
